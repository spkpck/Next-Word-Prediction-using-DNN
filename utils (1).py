...
def generate_sentence(model, index_to_word, word_to_index, min_length=2):
    # We start the sentence with the start toke
...
def generate_word(model,n, index_to_word, word_to_index, new_sent):
   sent = None
   count=1
   w = new_sent
   a=[]
   for i in range(n):
       ...
       while not sent:
           new_sentence = []
           splitinput = w.split()
           for each in splitinput:
               new_sentence.append(word_to_index[each.decode('utf-8')])
           sent = generate_words(model, index_to_word, word_to_index,new_sentence)
           if sent[1] not in a:
                a.insert(count,sent.__getitem__(1))
                count=count+1
                print_word(sent, index_to_word)
                sent=sent[0]
                break
           else:
                sent=None
...
def generate_words(model, index_to_word, word_to_index,input):
    # We start the sentence with the start token
    next_word_probs = model.predict(input)[-1]
    samples = np.random.multinomial(1, next_word_probs)
    for each in a:
        samples[each]=0 #code not being executed
    sampled_word = np.argmax(samples)
    input.append(sampled_word)
    input.append(SENTENCE_END_TOKEN)
    return input


def print_word(s, index_to_word):
